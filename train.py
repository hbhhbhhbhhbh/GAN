...
writer = SummaryWriter(f"/root/tf-logs/1")
...
dataloader = DataLoader(        ## 改成自己存放文件的目录
    ImageDataset("./data-pre/imgs", transforms_=transforms_, unaligned=True),  ## "./datasets/facades" , unaligned:设置非对其数据
...
val_dataloader = DataLoader(
    ImageDataset("./data-pre/imgs", transforms_=transforms_, unaligned=True, mode="val"), ## "./datasets/facades"
    batch_size=5,
    shuffle=True,
    num_workers=1,
)
...
def train():
    ...
    #       print('here is %d' % i)
            ...
            writer.add_scalar('Loss/GAN', loss_GAN.item(), batches_done)
            writer.add_scalar('Loss/Cycle', loss_cycle.item(), batches_done)
            writer.add_scalar('Loss/Identity', loss_identity.item(), batches_done)
            
            # 定期计算FID
            if batches_done % opt.sample_interval == 0:
                # 保存评估用图像
                save_image(real_A*0.5+0.5, "fid/real/real_A.png")
                save_image(fake_B*0.5+0.5, "fid/fake/fake_B.png")
                
                # 计算FID
                fid_value = calculate_fid("fid/real", "fid/fake")
                writer.add_scalar('Metrics/FID', fid_value, batches_done)
                
                # 记录生成图像
                writer.add_image('Generated/fake_B', fake_B[0]*0.5+0.5, batches_done)
                # 每训练100张就保存一组测试集中的图片
            if batches_done % opt.sample_interval == 0:
        ...
        if epoch % 5 == 0:
            fid_value = calculate_fid("fid/real", "fid/fake")
            writer.add_scalar('Metrics/Epoch_FID', fid_value, epoch)
